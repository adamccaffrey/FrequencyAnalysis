...
typedef struct _freqAnalysis_tilde
{
	...
	float			x_signal_in, tone, real, prevPhase[N], maxBinValue, 
					maxBinCentreFrequency, feedback, dynamicNorm, pitchThresh;
...
static void freqAnalysis_tilde_feedback(t_freqAnalysis *x, t_float f)
{
	...
	// x->dynamicNorm = NORM + (50.0 * NORM) * powf((1.0 - f), 2);
	// if (x->feedback >= 0.995) /*set timer and decrease NORM with time*/
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static t_int *freqAnalysis_tilde_perform(t_int *w)
{
	...
	while(blksize--)
	{
		...
		*out++ = (x->output[x->sampleCount] * NORM);// x->dynamicNorm);//* (1-1.0/(x->feedback * x->feedback)) * NORM  + 
...
static void *freqAnalysis_tilde_new()	
{
	...
	x->dynamicNorm = NORM + (50.0 * NORM) * powf((1.0 - x->feedback), 2);
...
void freqAnalysis_tilde_setup(void)
{
	...
	class_addmethod(freqAnalysis_tilde_class, (t_method)freqAnalysis_tilde_feedback, gensym("feedback"), A_FLOAT, 0);
